...
long long int predict(prediction_t& prediction) {
    prediction.sequence_predictions[prediction.sequence_predictions.size() - 1].push_back(0);
    for (int i = prediction.sequence_predictions.size() - 2; i >= 0; i--) {
        long long int new_last = prediction.sequence_predictions[i][prediction.sequence_predictions[i].size() - 1] + prediction.sequence_predictions[i + 1][prediction.sequence_predictions[i + 1].size() - 1];
        prediction.sequence_predictions[i].push_back(new_last);
    }
    return prediction.sequence_predictions[0][prediction.sequence_predictions[0].size() - 1];
}
